...
def is_valid_sudoku(board: list[list[int]]) -> bool:
        ...
        Args:
            board (list[list[int]]): A 9x9 nested list representing the Sudoku board.
                                    Each cell contains integers from 1-9.

        Returns:
            bool: True if the board is valid, False otherwise.

        Raises:
            ValueError: If the board dimensions are not valid (9x9) or contain invalid value types.
        """

        # Check if the board is proper size (9x9)
...
def check_valid_values(board: list[list[int]]) -> bool:
    valid_values = range(1, BOARD_SIZE+1)  # For board size of 9x9, valid values are 1 to 9

    for row in board:
        for value in row:
            if not isinstance(value, int) or value not in valid_values:
                raise ValueError("Board values must be integers between 1 and {}, value: {}.".format(BOARD_SIZE, value))
    return True
